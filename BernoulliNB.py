...
for a in alpha_list:
    clf = BernoulliNB(alpha=a, binarize=0.0, fit_prior=True, class_prior=None)
    ...
    #print("BernoulliNB accuracy: ", accuracy_score(valid_y, pred_y2))

accuracy_false_list,mul_logloss_false_list=[],[]
for a in alpha_list:
    clf = BernoulliNB(alpha=a, binarize=0.0, fit_prior=False, class_prior=None)
    clf.fit(train_x, train_y.values.ravel())
    pred_y = clf.predict_proba(valid_x)
    pred_y2 = clf.predict(valid_x)
    Multi_class_log_loss = logloss(pred_y, yij)
